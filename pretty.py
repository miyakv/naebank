...
def pretty_file(filename, **options):
    ...
    border_corner_bl        = options.pop("border_corner_bl", "╚═")
    border_corner_br        = options.pop("border_corner_br", "═╝")
    header                  = options.pop("header", True)
    border_header_separator = options.pop("border_header_separator", border_horizontal)
    border_header_left      = options.pop("border_header_left", '╠═')
    border_header_right     = options.pop("border_header_right", '═╣')
    ...
    with open(filename, "r", encoding='utf-8') as input: # parse the file and determine the width of each column
        reader = csv.reader(input, **options)
        for row in list(list(reader)):
            num_rows += 1
    ...
    with open(new_filename, "r", newline='', encoding='utf-8') as output:
        return ''.join(list(output.readlines()))
